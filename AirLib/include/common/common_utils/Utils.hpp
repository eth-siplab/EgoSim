...
#include "common/common_utils/stb_image_write.h"
...
//TODO: Use GCC and Clang version from https://github.com/scottt/debugbreak
    ...
    static void BGRAToRGB(const uint8_t* const image_data, int width, int height, uint8_t* const out_image_data)
    {
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
				int id = (i * width + j) * 3; // Pixel index
				int out_id = (i * width + j) * 3; // Pixel index

				out_image_data[out_id] = image_data[id + 2]; // R
				out_image_data[out_id + 1] = image_data[id + 1]; // G
				out_image_data[out_id + 2] = image_data[id]; // B
			}
		}
	}

    static void writePPMToJPGfile(const uint8_t* const image_data, int width, int height, const std::string& path)
    {

        std::string jpg = path.substr(0, path.size() - 3) + ".jpg";

        // convert BGR image to RGB
        uint8_t* rgb_image_data = new uint8_t[width * height * 3];
        BGRAToRGB(image_data, width, height, rgb_image_data);

        stbi_write_jpg(jpg.c_str(), width, height, 3, rgb_image_data, 90);
    }

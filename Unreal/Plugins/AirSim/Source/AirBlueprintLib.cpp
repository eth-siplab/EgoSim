...
bool UAirBlueprintLib::spawnPlayer(UWorld* context)
{
    ...
    // log function gets called
    LogMessageString("Player Start: ", "SuperStart", LogDebugLevel::Informational);
    // Found the following starts
    for (auto player_start : player_start_actors) {
        LogMessageString("Player Start: ", TCHAR_TO_UTF8(*player_start->GetName()), LogDebugLevel::Informational);
    }

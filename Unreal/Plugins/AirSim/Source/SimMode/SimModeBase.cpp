...
APlayerController* ASimModeBase::getRandomPlayerController() {
    ...
    APlayerController* RandomPlayerController = nullptr;
    if (PlayerControllers.Num() > 0)
    {
        // Select a random player controller
        int32 RandomIndex = FMath::RandRange(0, PlayerControllers.Num() - 1);
        RandomPlayerController = PlayerControllers[RandomIndex];
    }
...
void ASimModeBase::BeginPlay()
{
    ...
    if (have_existing_pawns) {
        ...
        APlayerController* player_controller = getRandomPlayerController();
...
void ASimModeBase::Tick(float DeltaSeconds)
{
    ...
    // get time 
    if (getSettings().runtimeTimeout > 0.0f && ClockFactory::get()->elapsedSince(ClockFactory::get()->getStart()) > getSettings().runtimeTimeout) {
        stopRecording();
        FGenericPlatformMisc::RequestExit(false);
        //UAirBlueprintLib::RunCommandOnGameThread([&]() {EndPlay(EEndPlayReason::EndPlayInEditor); }, true);
    }

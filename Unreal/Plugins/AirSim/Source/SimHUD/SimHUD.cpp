...
#include "Vehicles/SIPCharacter/SimModeSIPCharacter.h"
...
ASimHUD::ASimHUD()
{
    ...
    FCoreUObjectDelegates::PostLoadMapWithWorld.AddUObject(this, &ASimHUD::OnLevelLoaded);
}
...
void ASimHUD::OnLevelLoaded(UWorld* LoadedWorld)
{
    if (LoadedWorld == nullptr)
		return;

    UE_LOG(LogTemp, Warning, TEXT("OnLevelLoaded %s"), *LoadedWorld->GetName());
    try {
        /*UWorld* wo = GetWorld();
        setUnrealEngineSettings();
        createSimMode();
...
void ASimHUD::createSimMode()
{
    ...
    if (simmode_name == AirSimSettings::kSimModeTypeMultirotor)
        ...
        simmode_ = this->GetWorld()->SpawnActor<ASimModeComputerVision>(FVector::ZeroVector,
                                                                        FRotator::ZeroRotator,
                                                                        simmode_spawn_params);
    else if (simmode_name == AirSimSettings::kSimModeTypeSIPCharacter)
        simmode_ = this->GetWorld()->SpawnActor<ASimModeSIPCharacter>(FVector::ZeroVector,
                                                                        FRotator::ZeroRotator,
                                                                        simmode_spawn_params);

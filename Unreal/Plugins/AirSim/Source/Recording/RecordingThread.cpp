...
void processReadyFutures(std::vector<std::future<void>>& futures) {
    ...
    while (it != futures.end()) {
        ...
        if (status == std::future_status::ready) {
            try {
                // Ensures any stored exception is rethrown and caught here
                it->get();
            }
            ...
            // Remove the future from the collection once processed
            it = futures.erase(it);
        }
        else {
            ++it; // Move to the next future if this one is not ready
        }
    }
}
            ...
            if (interval_elapsed) {
                ...
                for (const auto& vehicle_sim_api : vehicle_sim_apis_) {
                    ...
                    if (!settings_.record_on_move || is_pose_unequal) {
                        ...
                        /*if (rec_fut != nullptr) {
                            futures.push_back(std::move(*rec_fut));
                        }*/
